...
namespace Meynell\BayesianBundle\Service\Classifier\Degenerator;

use Exception;

class StringDegenerator
{
    protected $multibyte;
    protected $encoding;
	...
	protected function degenerateWord($word)
	{
		...
		elseif($this->multibyte === TRUE) {
			# The multibyte upper/lower versions
			$lower = mb_strtolower($word, $this->encoding);
			$upper = mb_strtoupper($word, $this->encoding);
			$first = mb_substr($upper, 0, 1, $this->encoding) . mb_substr($lower, 1, mb_strlen($word), $this->encoding);
		}
